...
const FREE140_LENGTH_INSTRUCTION = `この週はX(旧Twitter)の無料プランの文字数制限に合わせて作成します。asaとtsujouのどちらも【全角140字以内】を最優先にしてください(140字を超えないこと)。
通常投稿(tsujou)は、500字版のような詳しい説明は入れず、伝えたい要点を1つだけに絞って短くまとめてください。実践ヒントは一言で十分です。絵文字は文字数を圧迫するので、朝投稿の「おはバルク💊」以外では基本的に使わないでください。`;
